...
import (
	...
	"terminus.io/dice/dice/internal/actionagent"
	"terminus.io/dice/dice/internal/openapi/api/apis/cmdb"
	"terminus.io/dice/dice/internal/openapi/api/apis/pipeline"
	...
	"terminus.io/dice/dice/pkg/expression"
...
continueContextVolumes:
	...
	task.Extra.OpenapiOAuth2TokenPayload = apistructs.OpenapiOAuth2TokenPayload{
		...
		AccessibleAPIs:       append(specYmlJob.AccessibleAPIs, pipeline.PIPELINE_PLATFORM_CALLBACK.Convert2AccessibleApi()),
	...
	tokenInfo, err := pre.Bdl.GetOpenapiOAuth2Token(apistructs.OpenapiOAuth2TokenGetRequest{
		...
		Payload: apistructs.OpenapiOAuth2TokenPayload{
			...
			AccessibleAPIs: []apistructs.AccessibleAPI{
				pipeline.PIPELINE_TASK_GET_BOOTSTRAP_INFO.Convert2AccessibleApi(),
				cmdb.CMDB_FILE_DOWNLOAD.Convert2AccessibleApi(),
				cmdb.CMDB_FILE_UPLOAD.Convert2AccessibleApi(),
			},
			Metadata: map[string]string{
				"pipelineID":            strconv.FormatUint(task.PipelineID, 10),
				"taskID":                strconv.FormatUint(task.ID, 10),
