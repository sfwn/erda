...
func getSnippetCaches(p *spec.Pipeline) ([]pipelineyml.SnippetPipelineYmlCache, error) {
	pipelineYml, err := pipelineyml.New(
		[]byte(p.PipelineYml),
	)
	...
	stages := ToSimplePipelineYmlActionSlice(pipelineYml.Spec())
	cache := pipelineyml.SnippetCache{}
	err = cache.InitCache(stages, p.Labels)
	if err != nil {
		return nil, err
	}

	return cache.PipelineYamlCaches, nil
}

func ToSimplePipelineYmlActionSlice(s *pipelineyml.Spec) [][]*apistructs.PipelineYmlAction {
	...
	s.LoopStagesActions(func(stageIndex int, action *pipelineyml.Action) {
		...
		stages[stageIndex] = append(stages[stageIndex], newAction)
	})
